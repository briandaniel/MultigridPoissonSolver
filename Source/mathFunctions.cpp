...
// Complex trapezoidal integration
CX CXtrapz(CX *F, int N, double dz)
{
	CX sum = 0;
	for (int k = 0; k < (N-1); k++)
	{
		sum = sum + (F[k] + F[k+1])/2.0;
	}
	CX summed = dz*sum;
	return summed;
}
...
// Normalizes PSI so that the integral of |PSI|^2 dz = 2 [i.e. density of each wavefunction is 2]
void sqNormalize( CX** PSI, int N, int Npsi, double dz )
{
	...
	for (int s = 0; s< Npsi; s++)
	{
		...
		for( int k = 0; k < N-1; k++)
		{
			rho[k] = pow( abs( PSI[s][k]), 2.0 ) ; 
		}

		double I = trapz(rho, N, dz );
	
		// Divide by integral value
		for( int k = 0; k < N; k++)
		{
			PSI[s][k] = PSI[s][k]/sqrt(I);
		}
	}
...
{
	d2F[0]	 = ( F[1] - 2.0*F[0] + F[N-1] ) / pow(dz,2.0);
	d2F[N-1] = ( F[0] - 2.0*F[N-1] + F[N-2] ) / pow(dz,2.0);
...
// Sorts M values of x in R^N using M function evaluations stored in y from lowest to highest
void xySort (double** x, double* y,  int M, int N)
{
	int min = 0;
	double **	xTemp = new double * [M];
	for (int k = 0; k< (M); k++ )
				xTemp[k] = new double [N];
